...
var newLogs={
  ...
  passMixinsToPosts(event){
    ...
    this.metaContentMixins={
      contentMixins: newLogs.contentMixins,
      getCollectContent: function(){
        return this.contentMixins.map(event => function(){
          return {
            currentEvent: {event},
            getEvent: function(){
    ...
    let {metaContentMixins}=this
    this.collectContent=metaContentMixins.getCollectContent()
  },
  insert(props){
    if (!this.props){
      this.props=props
