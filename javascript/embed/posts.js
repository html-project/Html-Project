...
document.write("<script>function alertFunction(){event={preventDefault: function(defulat){this.defaultModule=defulat;}, noexpect: function(){event.preventDefault({mixins: [0, 1]})}}} function alertAnotherFunction(){document.getElementById('myTextarea').innerHTML = ''; }</script><style>div {color: gray; background: #333;}</style><div class='gray'><!-- Note: This tag is obsolete.--><center><textarea id='myTextarea' placeholder='Insert posts here' rows='4' cols='50'></textarea>"+"<form action='#'><button type='submit' onclick='alertFunction()''> Submit the changes</button><button type='reset' onclick='alertAnotherFunction()'>Defaulting this change </button></form></center></div>")
